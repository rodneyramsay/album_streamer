...
sub restore_player {
    ...
    # navigate back to the starting album using A+Y
    ...
    for my $s (1 .. $steps) {
        ...
        unless ($after) {
            log_test("FAIL: A+Y did not change album on step $s");
            $failed++;
            return;
        }
    }
...
sub run_tests {
    log_test("--- Skip/track navigation end-to-end ---");
    ...
    # A+X -- next track
    log_test("--- A+X next track ---");
    my $before = play_pid();
    combo('A', 'X');
    my $after = wait_for_pid_change($before, 5.0);
    if ($after) {
        log_test("PASS: A+X changed play process ($before -> $after)");
        $passed++;
    } else {
        log_test("FAIL: A+X did not change play process");
        $failed++;
    }
    ...
    $before = play_pid();
    combo('A', 'Y');
    $after = wait_for_pid_change($before, 5.0);
    if ($after) {
        log_test("PASS: A+Y changed play process ($before -> $after)");
        $passed++;
    } else {
    ...
    $after = wait_for_pid_change($before, 5.0);
    if ($after) {
        log_test("PASS: B+Y changed play process ($before -> $after)");
        $passed++;
    } else {
    ...
    # B+X -- previous track / restart
    log_test("--- B+X previous track ---");
    $before = play_pid();
    combo('B', 'X');
    $after = wait_for_pid_change($before, 5.0);
    if ($after) {
        log_test("PASS: B+X changed play process ($before -> $after)");
        $passed++;
    } else {
        log_test("FAIL: B+X did not change play process");
        $failed++;
    }

    # B+X again after a few seconds -- should restart same track (back to start)
    log_test("--- B+X back-to-start after 5.5s ---");
    sleep(5.5);
    $before = play_pid();
    combo('B', 'X');
    $after = wait_for_pid_change($before, 5.0);
    if ($after) {
        log_test("PASS: B+X restarted play ($before -> $after)");
        $passed++;
    } else {
        log_test("FAIL: B+X did not restart play");
        $failed++;
    }
